...
for (let i in a){
  ...
  (ship.typespec.level != 1) && select_ships[ship.level-2].push({name:ship.name,code:ship.typespec.level*100+ship.typespec.model});
}
...
this.event = function (event,game){
  switch (event.name){
    ...
    case "ui_component_clicked":
      ...
      if (component == "ship1"){
        ship.set({type:round_ships[0].code,invulnerable:400,stats:88888888,idle:false,shield:999});
        ...
        ship.set({type:round_ships[1].code,invulnerable:400,stats:88888888,idle:false,shield:999});
        ship.set({crystals:((Math.round(6||0)**2)*20/4)});
        ship.setUIComponent({id:"ship1",visible:false});      
        ship.setUIComponent({id:"ship2",visible:false});      
        ship.setUIComponent({id:"ship text",visible:false});
...
function spawnSecondary(){
  ...
  var options = [10,10,10,11,11,20,21,91,91];
