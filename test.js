try
{
    ...
    function checkTicks (game)  // all commands for tick goes here
    {
        game.updateShips();
        game.checkForTimers();
    ...
    lOlO0.prototype.eventReceived = function(t) {
        ...
        if (null != t.data) {
            ...
            try {
                null != this.modding.context.event && (checkEvents(t.data, this.modding.game), this.modding.context.event(t.data, this.modding.game))
            } catch (t) {
                t
            }
            switch (t.data.name) {
